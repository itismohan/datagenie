...
from app.db.neo4j import get_settings
...
def require_mcp_gateway_actor(request: Request) -> McpActor:
    ...
    if not settings.mcp_gateway_service_identity_enabled:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail={"code": "mcp_service_unauthorized", "message": "MCP service delegation is disabled."})
    ...
    if not all([service_id, timestamp, actor_b64, signature]) or not hmac.compare_digest(service_id or "", settings.mcp_gateway_service_id):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail={"code": "mcp_service_unauthorized", "message": "A trusted MCP service identity is required."})
    try:
        ...
        decoded = json.loads(base64.urlsafe_b64decode((actor_b64 + "=" * (-len(actor_b64) % 4)).encode()))
        subject, tenant_id, roles, request_id = decoded.get("subject"), decoded.get("tenant_id"), decoded.get("roles"), decoded.get("request_id")
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail={"code": "mcp_service_unauthorized", "message": "The MCP service identity packet is invalid or expired."}) from exc
