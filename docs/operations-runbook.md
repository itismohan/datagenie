...
For a connector failure, inspect job history, cursor, retry lineage, cancellation flag, error, and source permissions. Retry only after addressing the causal condition. For quality incidents, inspect rule version, evidence, severity, assignee, and comments before changing thresholds or resolving the incident.

The current platform does not provide dead-letter queues or asynchronous connector workers. Repeated connector failure is a P0 reliability gate; do not rely on long-running harvests executing inside API requests at customer scale.[1]
